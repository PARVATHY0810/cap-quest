...
const loadDashboard = async (req, res) => {
  if (req.session.admin) {
    try {
      ...
      const prevMonthUsers = await User.countDocuments({ isAdmin: false, createdOn: { $lt: currentMonthStart } });
      const prevMonthProducts = await Product.countDocuments({ createdAt: { $lt: currentMonthStart } });
      const prevMonthOrders = await Order.countDocuments({ orderDate: { $gte: previousMonthStart, $lte: previousMonthEnd } });

      const prevMonthRevenue = (await Order.find({ orderDate: { $gte: previousMonthStart, $lte: previousMonthEnd } }))
        .reduce((sum, order) => sum + (order.finalAmount || 0), 0);

      const totalUsersGrowth = prevMonthUsers > 0 ? ((totalUsers - prevMonthUsers) / prevMonthUsers * 100).toFixed(1) : 5.3;
      const totalProductsGrowth = prevMonthProducts > 0 ? ((totalProducts - prevMonthProducts) / prevMonthProducts * 100).toFixed(1) : 7.1;
      const currentMonthOrders = await Order.countDocuments({ orderDate: { $gte: currentMonthStart } });
      const totalOrdersGrowth = prevMonthOrders > 0 ? ((currentMonthOrders / prevMonthOrders * 100) - 100).toFixed(1) : 3.2;
      const currentMonthRevenue = (await Order.find({ orderDate: { $gte: currentMonthStart } }))
        .reduce((sum, order) => sum + (order.finalAmount || 0), 0);
      const totalRevenueGrowth = prevMonthRevenue > 0 ? ((currentMonthRevenue / prevMonthRevenue * 100) - 100).toFixed(1) : 8.9;
      ...
      const formattedRecentOrders = recentOrders.map((order) => ({
        orderId: order.orderId,
        customerName: order.userId ? order.userId.name : "Unknown Customer",
        productName: order.orderedItems.length > 0 && order.orderedItems[0].product ? order.orderedItems[0].product.productName : "Unknown Product",
        amount: order.finalAmount,
        status: order.orderedItems[0]?.status || "Processing",
        date: order.orderDate.toLocaleDateString(),
      }));
    ...
    const groupedData = reportData.reduce((acc, curr) => {
      ...
      if (!acc[date]) {
        acc[date] = {
          ...
          revenue: 0,
          discount: 0,
          couponCode: curr.couponCode,
          couponApplied: curr.couponApplied,
          productsSold: 0,
        };
      }
      acc[date].orders += curr.orders;
      acc[date].revenue += curr.revenue;
      acc[date].discount += curr.discount;
      acc[date].productsSold += curr.productsSold;
      return acc;
    }, {});
...
const generatePDFReport = (data, reportType) => {
  ...
  doc.text("Discount", 550, tableTop);
  doc.text("Coupon Code", 650, tableTop);
  doc.text("Coupon Applied", 750, tableTop);

  doc.lineWidth(1)
    .moveTo(50, tableTop + 20)
    .lineTo(800, tableTop + 20)
    .stroke();

  let yPosition = tableTop + 40;
  data.forEach((row) => {
    doc.fontSize(10);
    doc.text(row.orderId || "", 50, yPosition, { width: 100 });
    doc.text(row.date || "", 150, yPosition, { width: 100 });
    doc.text(row.customerName || "Unknown", 250, yPosition, { width: 100 });
    doc.text(row.status || "", 350, yPosition, { width: 100 });
    doc.text(`₹${row.revenue.toLocaleString("en-IN")}`, 450, yPosition, { width: 100 });
    doc.text(`₹${row.discount.toLocaleString("en-IN")}`, 550, yPosition, { width: 100 });
    doc.text(row.couponCode || "N/A", 650, yPosition, { width: 100 });
    doc.text(row.couponApplied ? "Yes" : "No", 750, yPosition, { width: 100 });

    yPosition += 30;
    doc.lineWidth(0.5)
      .moveTo(50, yPosition - 5)
      .lineTo(800, yPosition - 5)
      .stroke();
  });
...
const generateExcelReport = async (data, reportType) => {
  ...
  worksheet.addRow(["Order ID", "Date", "Customer", "Status", "Amount", "Discount", "Coupon Code", "Coupon Applied"]);
  data.forEach((row) => {
    worksheet.addRow([row.orderId, row.date, row.customerName, row.status, row.revenue, row.discount, row.couponCode, row.couponApplied]);
  });
